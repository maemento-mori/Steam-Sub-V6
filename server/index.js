...
app.get('/search/:name', async (req, res) => {
  ...
  async function getIndividualMods(pageLinks) {
    ...
    const promises = pageLinks.map(async (pageLink) => {
      ...
      const uploadDateIDText = $$('.sectionTab.description').attr('href')
      const uploadDateID = uploadDateIDText.replace('https://steamcommunity.com/sharedfiles/filedetails/?id=', '')
    ...
    const modList = modDataArray.map((modData) => {
      const newMod = {
        ...
        uploadDateID: modData.uploadDateID,
      }
      return newMod
    })
